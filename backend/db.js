...
async function tornarAdmin(pedrozzMods) {
    await usuario.updateOne({ nome_usuario: pedrozzMods }, { admin: true });
}
module.exports.tornarAdmin = tornarAdmin;

    async function verificarAdmin(pedrozzMods) {
    let user = await usuario.findOne({ nome_usuario: pedrozzMods });
